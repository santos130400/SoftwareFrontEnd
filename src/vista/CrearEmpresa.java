    ...
    public void actionPerformed(ActionEvent evento) {
        ...
        if (evento.getSource() == this.jButton4) {
            if(jTextField2.getText().equals("") || jTextField3.getText().equals("") || jTextField4.getText().equals("") || (String.valueOf(jPasswordField1.getPassword())).equals("")){
                ...
                try {
                    InputStream response = new URL("http://localhost:8080/api/createEmpresa/"+usuario+"/"+password+"/"+NIT+"/"+nombre+"/"+direccion+"/"+key).openStream();
                    this.temp=IOUtils.toString(response, "UTF-8");
                    System.out.println(this.temp);
                    response.close();
                    InputStream response1 = new URL("http://localhost:8080/api/readEmpresalogin/"+usuario).openStream();
                    String te=IOUtils.toString(response1, "UTF-8");
